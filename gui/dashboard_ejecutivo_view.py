...
def _obtener_datos_dashboard(periodo_actual: datetime, tickets, compras, gastos, rentabilidad) -> dict:
    ...
    return {
        ...
        "compras_mes": len(compras_mes_actual),
        "gastos_mes": len(gastos_mes_actual),
...
def _render_dashboard(content_frame: ttk.Frame, datos: dict) -> None:
    ...
    ttk.Label(content_frame, text=f"Período: {datos['periodo']}", font=("Helvetica", 11)).pack(anchor="w", pady=(2, 2))
    ttk.Label(
        content_frame,
        text=(
            f"Datos reales del período → Tickets: {datos['tickets_mes']} | "
            f"Compras: {datos['compras_mes']} | Gastos: {datos['gastos_mes']}"
        ),
...
def agregar_tab_dashboard_ejecutivo(notebook: ttk.Notebook) -> None:
    ...
    ultimo_periodo_con_datos = periodos[-1]
    selected_year = tk.StringVar(value=str(ultimo_periodo_con_datos[0]))
    ...
    selected_month = tk.StringVar(value=str(ultimo_periodo_con_datos[1]))
